...
//!
//! This enables tests to find locally-copied binaries even when
//! CARGO_TARGET_DIR points to a shared location across worktrees.

use std::path::PathBuf;

fn main() {
    // CARGO_MANIFEST_DIR points to vibes-iggy/, workspace root is one level up
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let workspace_root = PathBuf::from(&manifest_dir)
        .parent()
        .expect("vibes-iggy should be in workspace root")
        .to_string_lossy()
        .to_string();
