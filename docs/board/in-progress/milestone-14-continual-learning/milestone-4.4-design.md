...
|-----------|-------|--------------|
| **4.4.1** | Infrastructure | Iggy subprocess management, event log trait + implementation, AssessmentProcessor skeleton, config structure |
| **4.4.2** | Assessment Logic | Lightweight/medium/heavy detection, circuit breaker, LLM integration, CLI commands |
...
### Milestone 4.4.1: Infrastructure

**Goal:** Iggy running, event log working, assessment processor skeleton in place.

**Deliverables:**

1. **Iggy Management**
   - `IggyManager` struct with subprocess lifecycle
   - Bundled binary distribution strategy
   - Health monitoring and auto-restart
   - Data directory setup (user/project scope)

2. **Event Log Abstraction**
   - `AssessmentLog` trait definition
   - `IggyAssessmentLog` implementation
   - Stream/topic setup: `groove.assessment.{lightweight,medium,heavy}`
   - Retention policy configuration

3. **Assessment Event Types**
   - `AssessmentContext` with full lineage
   - `LightweightEvent`, `MediumEvent`, `HeavyEvent`
   - Serialization (MessagePack or JSON for Iggy)

4. **AssessmentProcessor Skeleton**
   - Subscribe to EventBus
   - Fire-and-forget channel to Iggy writer task
   - Stub implementations for detection (pass-through)

5. **Configuration**
   - `[plugins.groove.assessment]` schema
   - Config loading with scope resolution
   - Integration with existing groove config

**Exit Criteria:**
- `vibes daemon` spawns Iggy successfully
- Events flow from EventBus → AssessmentProcessor → Iggy
- Can read events back from Iggy via CLI
- Config loading works at user and project scope

---

### Milestone 4.4.2: Assessment Logic

**Goal:** Full three-tier assessment with circuit breaker and interventions.

**Deliverables:**

1. **Lightweight Assessment**
   - `LightweightDetector` with regex pattern matching
   - Default patterns (positive/negative linguistic signals)
   - User-configurable pattern merging
   - Frustration/success EMA computation
   - Tool output parsing (build/test/lint signals)

2. **Medium Assessment**
   - `CheckpointManager` with trigger detection
   - `AssessmentLLM` trait definition
   - `HarnessLLM` implementation (subprocess)
   - `AnthropicLLM` implementation (API)
   - Segment summarization prompts
   - Token metrics computation

3. **Heavy Assessment**
   - `SessionEndDetector` trait + implementations (hook, timeout)
   - `SamplingStrategy` with boost conditions
   - Full session analysis prompts
   - Outcome classification
   - Attribution context population

4. **Circuit Breaker**
   - `CircuitBreakerAlgorithm` trait
   - `EmaCircuitBreaker` implementation
   - `InterventionMechanism` trait
   - `HookBasedIntervention` implementation
...
**Exit Criteria:**
- Lightweight signals detected on every message
- Medium checkpoints triggered and summarized
- Heavy assessment runs on session end (sampled)
- Circuit breaker intervenes when frustration spikes
- CLI shows assessment status and history
- All thresholds are `AdaptiveParam` (ready for 4.6 to update)
...
4.4.1 Infrastructure
    │
    ▼
4.4.2 Assessment Logic
    │
    ├──► 4.5 Learning Extraction (uses heavy assessment triggers)
    └──► 4.6 Attribution Engine (uses assessment events + context)
```
